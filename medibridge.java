...
// ============================================
// PART 0: THEME & UTILITIES
// ============================================

/**
 * centralized theme class containing application colors, fonts, 
 * and styling helper methods for consistent UI design.
 */
class Theme {
    // --- Colors ---
    ...
    public static final Color DANGER = new Color(239, 83, 80); // Red
    ...
    /**
     * Applies standard styling to JTables (headers, row height, colors).
     */
    ...
    /**
     * Applies standard styling to Input Fields (borders, fonts).
     */
...
/**
 * A JPanel with rounded corners for card-like layout.
 */
...
/**
 * A JButton with rounded corners and hover effects.
 */
    ...
    public RoundedButton(String text, Color color, Color hoverColor) {
        ...
        addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) {
                setModel(new DefaultButtonModel() { boolean isRollover = true; });
                repaint();
            }
            public void mouseExited(MouseEvent e) {
                setModel(new DefaultButtonModel() { boolean isRollover = false; });
                repaint();
            }
        });
    }
...
// ============================================
// PART 1: DATABASE CONNECTION & MODELS
// ============================================
...
class User {
    ...
    User(int id, String email, String name, String phone, String type) {
        this.id = id;
        this.email = email;
        this.name = name;
        this.phone = phone;
...
class Patient extends User {
    ...
    Patient(int id, String email, String name, String phone) {
        super(id, email, name, phone, "PATIENT");
    }
}
...
class Doctor extends User {
    ...
    Doctor(int id, String email, String name, String phone, String spec, int exp) {
        super(id, email, name, phone, "DOCTOR");
...
class Appointment {
    ...
    int patientId;
    int doctorId;
    String patientName, doctorName, date, time, status, symptoms;
}
...
class Database {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (type.equals("PATIENT")) {
                    ...
                    ResultSet rs2 = ps2.executeQuery();
                    if (rs2.next()) {
                        p.patientId = rs2.getInt("patient_id");
                        p.gender = rs2.getString("gender");
                        p.address = rs2.getString("address");
                    }
                    con.close();
                    return p;
                    ...
                    ResultSet rs2 = ps2.executeQuery();
                    if (rs2.next()) {
                        d.doctorId = rs2.getInt("doctor_id");
                        d.specialization = rs2.getString("specialization");
                        d.experience = rs2.getInt("experience_years");
                        d.fee = rs2.getDouble("consultation_fee");
    ...
    static ArrayList<Doctor> getVerifiedDoctors() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                d.doctorId = rs.getInt("doctor_id");
    ...
    static String registerDoctor(String email, String pass, String name, String phone, String spec, int exp, String lic, double fee) {
        try {
            ...
            ps2.setInt(1, uid);
            ps2.setString(2, spec);
            ps2.setInt(3, exp);
            ps2.setString(4, "MBBS");
            ps2.setString(5, lic);
            ps2.setDouble(6, fee);
    ...
    // --- Appointment & Prescription Methods ---
    ...
    static ArrayList<Appointment> getDoctorAppointments(int did) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                a.time = rs.getString("appointment_time");
                a.status = rs.getString("status");
                a.symptoms = rs.getString("symptoms");
                list.add(a);
            }
            c.close();
        } catch (Exception e) {}
    ...
    static ArrayList<String[]> getPatientPrescriptions(int pid) {
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next())
                list.add(new String[]{rs.getString("medicine_name"), rs.getString("dosage") + " (" + rs.getString("frequency") + ")", rs.getString("duration"), "Dr. " + rs.getString("dr_name")});
            c.close();
        } catch (Exception e) {}
        return list;
    }
...
// ============================================
// PART 3: UI SCREENS (LOGIN/REGISTER)
// ============================================
...
class LoginScreen extends JFrame {
    ...
    JRadioButton patientBtn, doctorBtn;

    LoginScreen() {
        setTitle("MediBridge Login");
        setSize(450, 600);
        ...
        card.setBounds(50, 50, 340, 460);
        card.setLayout(null);
        add(card);

        // Headers
        JLabel t = new JLabel("MediBridge");
        t.setFont(Theme.FONT_TITLE);
        t.setForeground(Theme.PRIMARY);
        t.setBounds(30, 20, 200, 35);
        card.add(t);

        JLabel st = new JLabel("Healing starts here");
        st.setFont(Theme.FONT_REGULAR);
        st.setForeground(Theme.TEXT_MED);
        st.setBounds(30, 55, 200, 20);
        card.add(st);

        // Form Fields
        int y = 90;
        card.add(lbl("Email", 30, y));
        emailBox = new JTextField();
        Theme.styleTextField(emailBox);
        emailBox.setBounds(30, y + 25, 280, 35);
        card.add(emailBox);
        y += 70;

        card.add(lbl("Password", 30, y));
        passBox = new JPasswordField();
        Theme.styleTextField(passBox);
        passBox.setEchoChar('•');
        passBox.setBounds(30, y + 25, 280, 35);
        card.add(passBox);
        y += 50;

        // User Type Selection
        patientBtn = new JRadioButton("Patient", true);
        styleRadio(patientBtn, 30, y + 20);
        doctorBtn = new JRadioButton("Doctor");
        styleRadio(doctorBtn, 150, y + 20);
        ...
        // Buttons
        RoundedButton login = new RoundedButton("Login", Theme.PRIMARY, Theme.PRIMARY_HOVER);
        login.setBounds(30, y, 280, 40);
        login.addActionListener(e -> doLogin());
        card.add(login);
        y += 50;

        JButton reg = new JButton("Create Account");
        reg.setFont(Theme.FONT_SMALL);
        reg.setForeground(Theme.ACCENT_BLUE);
        reg.setContentAreaFilled(false);
        reg.setBorderPainted(false);
        reg.setBounds(30, y, 280, 30);
        ...
        admin.setFont(Theme.FONT_SMALL);
        admin.setForeground(Theme.TEXT_MED);
        admin.setContentAreaFilled(false);
        admin.setBorderPainted(false);
        admin.setBounds(120, 530, 200, 30);
    ...
    void styleRadio(JRadioButton r, int x, int y) {
        r.setOpaque(false);
        r.setFont(Theme.FONT_REGULAR);
        r.setForeground(Theme.TEXT_DARK);
        r.setBounds(x, y, 100, 30);
    ...
    void doLogin() {
        ...
        if (u != null) {
            dispose();
            if (u instanceof Patient) new PatientDashboard((Patient) u);
            else if (u instanceof Doctor) new DoctorDashboard((Doctor) u);
        } else {
            JOptionPane.showMessageDialog(this, "Login Failed. Ensure account is Verified.");
        }
    }
}

class RegisterScreen extends JFrame {
    JTextField nameF, emailF, phoneF, addrF, feeF, licF, expF;
    JPasswordField passF;
    JComboBox<String> genderBox, specBox;
    JRadioButton pBtn, dBtn;
    JPanel docPanel;

    RegisterScreen() {
        setTitle("Join MediBridge");
        setSize(500, 750);
        setLocationRelativeTo(null);
        getContentPane().setBackground(Theme.BG_MAIN);
        setLayout(null);

        RoundedPanel card = new RoundedPanel(Color.WHITE);
        card.setBounds(30, 20, 425, 670);
        card.setLayout(null);
        add(card);
        ...
        t.setFont(Theme.FONT_TITLE);
        t.setForeground(Theme.PRIMARY);
        t.setBounds(30, 15, 300, 35);
        card.add(t);

        int y = 60;
        pBtn = new JRadioButton("Patient", true);
        styleRadio(pBtn, 30, y);
        dBtn = new JRadioButton("Doctor");
        styleRadio(dBtn, 140, y);
        ...
        nameF = addF(card, "Full Name", y); y += 50;
        emailF = addF(card, "Email", y); y += 50;
        phoneF = addF(card, "Phone", y); y += 50;

        JLabel pl = new JLabel("Password");
        pl.setFont(Theme.FONT_HEADER);
        pl.setForeground(Theme.TEXT_MED);
        pl.setBounds(30, y, 100, 20);
        card.add(pl);
        passF = new JPasswordField();
        Theme.styleTextField(passF);
        passF.setBounds(140, y, 250, 30);
        card.add(passF);
        y += 50;

        JLabel gl = new JLabel("Gender");
        gl.setFont(Theme.FONT_HEADER);
        gl.setForeground(Theme.TEXT_MED);
        gl.setBounds(30, y, 100, 20);
        card.add(gl);
        genderBox = new JComboBox<>(new String[]{"Male", "Female", "Other"});
        genderBox.setBounds(140, y, 250, 30);
        card.add(genderBox);
        y += 50;

        addrF = addF(card, "Address", y); y += 50;

        // Special Panel for Doctor Details
        docPanel = new JPanel();
        docPanel.setLayout(null);
        docPanel.setBackground(new Color(248, 250, 252));
        docPanel.setBounds(20, y, 380, 160);
        docPanel.setBorder(new LineBorder(Theme.PRIMARY, 1, true));
        docPanel.setVisible(false);

        int dy = 10;
        JLabel sl = new JLabel("Specialty:");
        sl.setBounds(10, dy, 80, 25);
        docPanel.add(sl);
        specBox = new JComboBox<>(Theme.SPECIALIZATIONS);
        specBox.setBounds(90, dy, 270, 25);
        docPanel.add(specBox);
        dy += 35;

        expF = addInner(docPanel, "Exp (Yrs):", dy); dy += 35;
        licF = addInner(docPanel, "License #:", dy); dy += 35;
        feeF = addInner(docPanel, "Fee (₹):", dy);

        card.add(docPanel);

        // Toggle doctor fields based on radio selection
        pBtn.addActionListener(e -> docPanel.setVisible(false));
        dBtn.addActionListener(e -> docPanel.setVisible(true));

        RoundedButton sub = new RoundedButton("Register", Theme.PRIMARY, Theme.PRIMARY_HOVER);
        sub.setBounds(30, 600, 180, 40);
        sub.addActionListener(e -> register());
        card.add(sub);

        JButton back = new JButton("Cancel");
        back.setBounds(230, 600, 100, 40);
    ...
    JTextField addF(JPanel p, String l, int y) {
        ...
        t.setBounds(140, y, 250, 30);
        p.add(t);
        return t;
    }

    JTextField addInner(JPanel p, String l, int y) {
        JLabel lbl = new JLabel(l);
        lbl.setBounds(10, y, 80, 25);
        p.add(lbl);
        JTextField t = new JTextField();
        Theme.styleTextField(t);
        t.setBounds(90, y, 270, 25);
        p.add(t);
        return t;
    }
    ...
    void register() {
        String res;
        if (pBtn.isSelected()) {
            res = Database.registerPatient(emailF.getText(), new String(passF.getPassword()), nameF.getText(), phoneF.getText(), (String) genderBox.getSelectedItem(), addrF.getText());
        } else {
            res = Database.registerDoctor(emailF.getText(), new String(passF.getPassword()), nameF.getText(), phoneF.getText(), (String) specBox.getSelectedItem(), Integer.parseInt(expF.getText()), licF.getText(), Double.parseDouble(feeF.getText()));
        }
...
class PatientDashboard extends JFrame {
    ...
    PatientDashboard(Patient p) {
        ...
        side.setBorder(BorderFactory.createMatteBorder(0, 0, 0, 1, new Color(230, 230, 230)));

        JLabel n = new JLabel("Hi, " + p.name.split(" ")[0]);
        n.setFont(Theme.FONT_TITLE);
        n.setForeground(Theme.PRIMARY);
        n.setBorder(new EmptyBorder(30, 20, 20, 0));
        side.add(n);
    ...
    void update(String t, JComponent c) {
        content.removeAll();
    ...
    void findDocs() {
        ...
        top.setOpaque(false);
        JComboBox<String> specFilter = new JComboBox<>(Theme.SPECIALIZATIONS);
        specFilter.insertItemAt("All Specializations", 0);
        specFilter.setSelectedIndex(0);
        specFilter.setPreferredSize(new Dimension(250, 35));
        top.add(new JLabel("Filter by: "));
        top.add(specFilter);
        ...
        for (Doctor d : docs) {
            model.addRow(new Object[]{d.doctorId, "Dr. " + d.name, d.specialization, d.experience + " yrs", "₹" + d.fee, "Book"});
        }
        ...
        specFilter.addActionListener(e -> {
            String txt = (String) specFilter.getSelectedItem();
            if (txt.equals("All Specializations")) sorter.setRowFilter(null);
            else sorter.setRowFilter(RowFilter.regexFilter(txt));
        });
        ...
        table.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                if (table.getSelectedColumn() == 5) {
                    ...
                    book((int) model.getValueAt(row, 0), (String) model.getValueAt(row, 1));
                }
            }
        });
        p.add(new JScrollPane(table));
        update("Find Specialists", p);
    }

    void book(int did, String dname) {
        JDialog d = new JDialog(this, "Book Appointment", true);
        d.setSize(400, 450);
        d.setLocationRelativeTo(this);
        d.setLayout(null);
        d.getContentPane().setBackground(Color.WHITE);

        JLabel t = new JLabel("Book: " + dname);
        t.setFont(Theme.FONT_TITLE);
        t.setForeground(Theme.PRIMARY);
        t.setBounds(20, 10, 300, 30);
        d.add(t);
        ...
        dateSpin.setBounds(20, 80, 150, 30);
        d.add(dateSpin);
        ...
        timeSpin.setBounds(200, 80, 150, 30);
        d.add(timeSpin);

        d.add(new JLabel("Symptoms:") {{ setBounds(20, 130, 100, 20); }});
        JTextArea symp = new JTextArea();
        symp.setBorder(new LineBorder(Color.LIGHT_GRAY));
        symp.setBounds(20, 150, 340, 100);
        d.add(symp);

        RoundedButton b = new RoundedButton("Confirm", Theme.PRIMARY, Theme.PRIMARY_HOVER);
        b.setBounds(100, 300, 200, 40);
        b.addActionListener(e -> {
            // FIX: Mandatory Symptoms Check
            if (symp.getText().trim().isEmpty()) {
                JOptionPane.showMessageDialog(d, "Please tell your symptoms before booking.");
                return;
            }
            Date dt = (Date) dateSpin.getValue();
            Date tm = (Date) timeSpin.getValue();
            if (Database.bookAppointment(patient.patientId, did, new SimpleDateFormat("yyyy-MM-dd").format(dt), new SimpleDateFormat("HH:mm:00").format(tm), symp.getText())) {
                JOptionPane.showMessageDialog(d, "Booked! Waiting for Doctor Approval.");
                d.dispose();
                showAppts();
            } else {
                JOptionPane.showMessageDialog(d, "Failed.");
            }
        });
        d.add(b);
        d.setVisible(true);
    ...
    void showAppts() {
        String[] c = {"ID", "Doctor", "Date", "Time", "Status"};
        DefaultTableModel m = new DefaultTableModel(c, 0);
        for (Appointment a : Database.getPatientAppointments(patient.patientId)) {
            m.addRow(new Object[]{a.id, a.doctorName, a.date, a.time, a.status});
        }
        JTable t = new JTable(m);
        Theme.styleTable(t);
        update("My Appointments", new JScrollPane(t));
    }

    void showRx() {
        String[] c = {"Medicine", "Dosage", "Duration", "Prescribed By"};
        DefaultTableModel m = new DefaultTableModel(c, 0);
        for (String[] s : Database.getPatientPrescriptions(patient.patientId)) {
            m.addRow(s);
        }
        JTable t = new JTable(m);
        Theme.styleTable(t);
        update("My Prescriptions", new JScrollPane(t));
    }
}
...
class DoctorDashboard extends JFrame {
    ...
    DoctorDashboard(Doctor d) {
        ...
        JLabel l = new JLabel("Dr. " + d.name);
        l.setFont(Theme.FONT_TITLE);
        l.setForeground(Theme.PRIMARY);
        l.setBorder(new EmptyBorder(30, 10, 20, 0));
        side.add(l);

        JButton b1 = new JButton("Appointments");
        b1.setFont(Theme.FONT_NAV);
        b1.setBackground(Color.WHITE);
        b1.setBorderPainted(false);
        b1.addActionListener(e -> showAppts());
        side.add(b1);

        JButton b2 = new JButton("Logout");
        b2.setFont(Theme.FONT_NAV);
        b2.setBackground(Color.WHITE);
        b2.setForeground(Theme.DANGER);
    ...
    void showAppts() {
        content.removeAll();
        String[] c = {"ID", "Patient", "Date", "Time", "Status", "Action", "Rx", "History"};
        DefaultTableModel m = new DefaultTableModel(c, 0);

        ArrayList<Appointment> appts = Database.getDoctorAppointments(doc.doctorId);
        for (Appointment a : appts) {
            m.addRow(new Object[]{a.id, a.patientName, a.date, a.time, a.status, "Accept/Decline", "Prescribe", "View History"});
        }
        ...
        t.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                ...
                if (r != -1) {
                    ...
                    // FIX: View Symptoms before Action
                    ...
                    if (t.getSelectedColumn() == 6) {
                        if (m.getValueAt(r, 4).toString().equals("ACCEPTED")) {
                            ...
                            JOptionPane.showMessageDialog(null, "Accept appointment first!");
                        }
                    }
                    ...
                    if (t.getSelectedColumn() == 7) {
                        ...
                        JTextArea ta = new JTextArea(hist);
                        ta.setFont(Theme.FONT_REGULAR);
        ...
        content.add(new JLabel(" Appointments") {{ setFont(Theme.FONT_TITLE); }}, BorderLayout.NORTH);
    ...
    void writeRx(int aptId, int patId) {
        JDialog d = new JDialog(this, "Write Prescription", true);
        d.setSize(400, 400);
        d.setLocationRelativeTo(this);
        d.setLayout(null);
        d.getContentPane().setBackground(Color.WHITE);

        JLabel t = new JLabel("New Prescription");
        t.setFont(Theme.FONT_TITLE);
        t.setForeground(Theme.PRIMARY);
        t.setBounds(20, 10, 300, 30);
        d.add(t);
        ...
        RoundedButton b = new RoundedButton("Save", Theme.PRIMARY, Theme.PRIMARY_HOVER);
        b.setBounds(100, y, 200, 40);
        b.addActionListener(e -> {
            if (Database.addPrescription(aptId, patId, doc.doctorId, mF.getText(), dF.getText(), fF.getText(), duF.getText())) {
                JOptionPane.showMessageDialog(d, "Prescription Sent!");
                d.dispose();
            } else {
                JOptionPane.showMessageDialog(d, "Error sending.");
            }
        });
        d.add(b);
        d.setVisible(true);
    ...
    JTextField f(JDialog d, String l, int y) {
        JLabel lb = new JLabel(l);
        lb.setBounds(20, y, 100, 25);
        d.add(lb);
        JTextField t = new JTextField();
        Theme.styleTextField(t);
        t.setBounds(120, y, 240, 30);
        d.add(t);
        return t;
    }
}
...
class AdminLogin extends JFrame {
    AdminLogin() {
        ...
        l.setFont(Theme.FONT_TITLE);
        l.setBounds(110, 30, 200, 30);
        add(l);

        JTextField u = new JTextField("admin@medibridge.com");
        Theme.styleTextField(u);
        u.setBounds(50, 80, 280, 35);
        add(u);

        JPasswordField p = new JPasswordField("admin123");
        Theme.styleTextField(p);
        p.setBounds(50, 130, 280, 35);
        add(p);

        RoundedButton b = new RoundedButton("Login", Theme.PRIMARY, Theme.PRIMARY_HOVER);
        b.setBounds(100, 190, 180, 40);
        b.addActionListener(e -> {
            if (Database.login(u.getText(), new String(p.getPassword()), "ADMIN") != null) {
                new AdminDashboard();
                dispose();
            } else {
...
class AdminDashboard extends JFrame {
    ...
    AdminDashboard() {
        ...
        side.setBackground(Color.WHITE);
        side.setBorder(BorderFactory.createMatteBorder(0, 0, 0, 1, Color.LIGHT_GRAY));

        JLabel t = new JLabel("Admin Panel");
        t.setFont(Theme.FONT_TITLE);
        t.setForeground(Theme.PRIMARY);
        t.setBorder(new EmptyBorder(30, 10, 30, 10));
        side.add(t);

        JButton btnDocs = new JButton("Manage Doctors");
        btnDocs.setPreferredSize(new Dimension(200, 40));
        btnDocs.addActionListener(e -> loadDocs());
        side.add(btnDocs);

        JButton btnPats = new JButton("View Patients");
        btnPats.setPreferredSize(new Dimension(200, 40));
        btnPats.addActionListener(e -> loadPats());
        side.add(btnPats);

        JButton btnLogs = new JButton("System Logs");
        btnLogs.setPreferredSize(new Dimension(200, 40));
        btnLogs.addActionListener(e -> loadLogs());
        side.add(btnLogs);

        JButton btnOut = new JButton("Logout");
        btnOut.setPreferredSize(new Dimension(200, 40));
        btnOut.setForeground(Theme.DANGER);
    ...
    void loadDocs() {
        ...
        contentPanel.add(new JLabel("Doctor Verification") {{ setFont(Theme.FONT_TITLE); }}, BorderLayout.NORTH);
        String[] cols = {"Doc ID", "Name", "Specialization", "Verified", "Action"};
        ...
        try {
            Connection c = DB.connect();
            ResultSet rs = c.createStatement().executeQuery("SELECT d.doctor_id, u.full_name, d.specialization, d.is_verified FROM doctors d JOIN users u ON d.user_id=u.user_id");
            while (rs.next()) {
                model.addRow(new Object[]{rs.getInt(1), rs.getString(2), rs.getString(3), rs.getBoolean(4), "Toggle Verify"});
            }
            c.close();
        } catch (Exception e) {}
        ...
        table.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                if (table.getSelectedColumn() == 4) {
                    ...
                    boolean status = (boolean) model.getValueAt(r, 3);
                    try {
                        Connection c = DB.connect();
    ...
    void loadPats() {
        contentPanel.removeAll();
        contentPanel.add(new JLabel("Registered Patients") {{ setFont(Theme.FONT_TITLE); }}, BorderLayout.NORTH);
        String[] cols = {"ID", "Name", "Email", "Phone", "Gender"};
        DefaultTableModel model = new DefaultTableModel(cols, 0);
        ArrayList<Patient> pats = Database.getAllPatients();
        for (Patient p : pats) {
            model.addRow(new Object[]{p.patientId, p.name, p.email, p.phone, p.gender});
        }
        JTable table = new JTable(model);
        Theme.styleTable(table);
        contentPanel.add(new JScrollPane(table), BorderLayout.CENTER);
